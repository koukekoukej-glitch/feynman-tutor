...
在 6 个核心场景上的评测，涵盖概念学习、零基础用户、日常知识、材料学习（YouTube/网页/B站）和错误处理：

| 指标 | 结果 |
|------|------|
| 有 Skill 通过率 | **100%**（14/14 检查点） |
| 无 Skill 通过率 | 19.4%（3/14 检查点） |
| **Skill 带来的提升** | **+80.6 百分点** |

**没有这个 Skill 时**，Claude 倾向于直接给出解释（被动学习模式）；**有了 Skill 后**，Claude 会执行完整的认知探测 → 角色反转 → 诊断 → 补强流程（主动学习模式）。
