...
- **找到了**——按下面的"扩展加载"逻辑收集相关笔记，一起读完。
- **他没聊过这个主题，但你能判断出属于哪个领域**——读 `domains/{领域名}/domain.md`，并把该领域内 `last_updated` 最近的 1-2 篇话题笔记也带上。陌生主题恰好是最该建跨域连接的时机，不要直接放弃加载。
...
1. **必读**：T 的话题笔记 + T 的 `domain` 字段指向的 `domains/{领域}/domain.md`

2. **邻居候选池**（去重后按下面的优先级取前 5 个加载）：

   - **出向**：T 的 frontmatter `related-topics` 和 `spontaneous-calls` 列出的所有话题
   - **入向（最容易漏的一步）**：用 Grep 在 `notes/domains/**/*.md` 里搜 `related-topics:` 和 `spontaneous-calls:` 列表里**包含 T 的文件名（不带 `.md`）**的笔记。这一步把"别人列了 T 但 T 没列别人"的笔记捞回来
   - **跨域模式入向**：用 Grep 在 `notes/cross-domain/*.md` 正文里搜 mention 了 T 的话题中文名或英文 slug 的模式

3. **优先级（高→低）**——超出预算时按这个裁剪：

   - (a) **双向命中**：既在出向也在入向——最强相关，必读
   - (b) **`spontaneous-calls` 命中**：他在某场对话里实际重走了 T 和它之间的推导——这是行为证据，比 `related-topics`（你当时的判断）强
   - (c) **时间相邻**：`last_updated` 在 T 的 `last_updated` ±7 天内——很可能是同一波思考线索
   - (d) **`related-topics` 单向命中**

4. **跨域模式**（最多 2 个）：T 的 `cross-domain-patterns` 列出的模式 + 反向 mention 命中的模式
